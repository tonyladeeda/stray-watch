...
export default function MasterProfileWizard({ session, onComplete, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await supabase.from('directory_profiles').upsert([{
      user_id: session.user.id,
      type: 'Foster',
      email: session.user.email,
      availability_status: 'Available',
      foster_details: formData, // Dumps the entire state object into JSONB
      is_approved: false
    }], { onConflict: 'user_id' });
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 z-50 overflow-y-auto flex flex-col font-sans text-slate-900">
      
      {/* Header & Progress Bar */}
      <div className="bg-white sticky top-0 z-10 border-b border-slate-200 shadow-sm px-4 py-4 flex flex-col gap-4">
        <div className="flex justify-between items-center max-w-2xl mx-auto w-full">
          <button onClick={onCancel} className="text-sm font-bold text-slate-400 hover:text-slate-600">Cancel</button>
          <h2 className="font-black text-lg text-slate-800">Master Profile Setup</h2>
          <span className="text-sm font-bold text-cyan-600">Step {step} of 5</span>
        </div>
        
        {/* Progress Track */}
        <div className="max-w-2xl mx-auto w-full flex justify-between relative">
      ...
      <div className="flex-grow p-4">
        <form onSubmit={step === 5 ? handleSubmit : handleNext} className="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4 mb-6">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Complete once. Apply anywhere.</p>
            </div>
          </div>
